...
- **`src/components/`**: UI building blocks.
  ...
  - **Feature Components**: `DocumentReader.jsx` provides the shared Code/Transparency reader shell. Specialized UI includes `DecisionTree.jsx` (interactive logic), `DefinitionPopup.jsx` (Code glossary), `FullTextSection.jsx` (legal text, Q&A, citation, resource-link, and bookmark rendering), `TransparencyLandingPage.jsx` (publication and unit cards), and `TableOfContents.jsx` (navigation).
