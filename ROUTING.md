# URL Routing Guide

This guide explains the app's URLs, the compatibility rules for links that have already been shared, and the checks to run after changing navigation or content identifiers.

The routing system is intentionally small and does not use a third-party routing library.

## URL Formats

| Destination | URL format | Example |
|---|---|---|
| Home Hub | `/` | `https://mte-code.org/` |
| Code index | `/code` | `https://mte-code.org/code` |
| Code chapter | `/code/:chapterId` | `https://mte-code.org/code/ch1` |
| Exact Code section | `/code/:chapterId#section-id` | `https://mte-code.org/code/ch1#ch1-2-event-location-and-venue` |
| Decision-tree index | `/trees` | `https://mte-code.org/trees` |
| Decision tree | `/trees/:treeId` | `https://mte-code.org/trees/dt-ch1-event-location` |
| Knowledge Quiz | `/quiz` | `https://mte-code.org/quiz` |
| Shared Quiz | `/quiz#quiz?q=questionIds` | `https://mte-code.org/quiz#quiz?q=q1,q2` |
| TPPT Checker | `/tppt` | `https://mte-code.org/tppt` |

Browser Back and Forward restore these app-level destinations. Individual steps inside a decision tree or an active Quiz are not separate history entries.

Scrolling through a Code chapter does not continuously change the address bar. Section anchors are used for direct links without creating a new browser-history entry for every section that passes through the viewport.

## Existing Links Remain Supported

Links created before path routing continue to work. When one is opened, the app resolves it and replaces it with the canonical URL without adding an unnecessary browser-history entry.

| Existing link | Canonical result |
|---|---|
| `/#ch1` | `/code/ch1` |
| `/#ch1-2-event-location-and-venue` | `/code/ch1#ch1-2-event-location-and-venue` |
| `/#dt-ch1-event-location` | `/trees/dt-ch1-event-location` |
| `/#quiz?q=q1,q2` | `/quiz#quiz?q=q1,q2` |
| `/#tppt` | `/tppt` |

Do not remove this compatibility behavior unless every published link has a deliberate migration path.

## Identifiers That Protect Links

Chapter routes use each chapter's `id` in `src/data/codeData.json`. Decision-tree routes use each tree's `id` in `src/data/treeData.json`. Renaming either value changes its public URL.

Code section anchors are generated by `generateSectionId()` from the chapter ID, section title, and section position. This means changing a section title can change its public URL and can also affect existing bookmarks. An empty section title uses its position, so reordering untitled sections can have the same effect.

Before renaming a chapter ID, tree ID, or section title:

1. Confirm that the content change is necessary and correct.
2. Treat the old URL as a compatibility requirement.
3. Have a developer add and test an explicit compatibility rule if the old URL must continue working.
4. Run the checks below before release.

The bookmark and Recently Viewed storage formats are independent from the browser-history implementation and must not be migrated merely because URL routing changes.

## Implementation Map

- `src/utils/routeUtils.js` contains pure URL builders and parsing rules.
- `src/config/routes.js` creates the parser from the current Code and decision-tree data.
- `src/hooks/useAppRouting.js` synchronizes React state with the address bar and handles Back, Forward, current navigation, and legacy URL replacement.
- `src/App.jsx` passes route-aware navigation callbacks to the existing components.
- `src/components/FullTextSection.jsx` builds canonical links and citation URLs for Code sections.
- `src/components/quiz/QuizContent.jsx` loads shared Quiz URLs and responds when Back or Forward changes the shared challenge.
- `src/components/quiz/QuizResults.jsx` builds canonical shared Quiz URLs.
- `tests/routeUtils.test.mjs` exercises every current chapter, generated section anchor, decision tree, and supported legacy format.

Keeping URL construction in these files prevents individual components from inventing different link formats.

## Hosting Requirement

A fresh request for a nested URL such as `/code/ch1` must return the React app's `index.html`; otherwise direct links and browser refreshes will produce a server 404.

The current Cloudflare Worker in `server.js` already provides this fallback for extensionless app paths after handling its reserved server routes. If the app is moved to another host, configure that host with the equivalent single-page-app fallback before deployment.

## Verification

From the app root folder, run:

```powershell
npm test
npm run check
```

`npm test` exercises the route parser against current project data and confirms that current section URLs are unique. `npm run check` also validates duplicate content IDs, runs type checks, and creates a production build. See `PROJECT_CHECKS.md` for plain-language instructions and troubleshooting.

Before a release that changes navigation or identifiers, also check manually:

1. Open `/code`, select two chapters, and confirm the URL changes each time.
2. Use browser Back and Forward and confirm both the URL and visible chapter are restored.
3. Copy a Code section link and open it in a fresh tab. Confirm the correct chapter and section appear.
4. Open one legacy chapter or section hash and confirm it changes to the canonical URL.
5. Open and refresh a decision-tree URL, a shared Quiz URL, and `/tppt`.
6. Refresh a nested chapter URL in the production preview or deployed app to confirm the hosting fallback works.

Do not release a route change if an existing link resolves to the wrong content, even when the automated checks pass.
